...
int max_time_treedepth = 60 * 60;  // A time limit of TEN minuts for now.
...
std::tuple<int, int, int> treedepth(const Graph &G, int search_lbnd,
  ...
  if (node == nullptr) {
    ...
    if (G.N > 25) {
      auto [data, isom] = hash_cache.Search(G.adj, 15);
      if (data) {
        std::cout << "Hash cache hit of size " << G.N << std::endl;
        node->lower_bound = lower = data->td;
        node->upper_bound = upper = data->td;
        node->root = root = G.global[isom[data->root]];
        return {lower, upper, root};
      }
    }
  }
  ...
  while (sep_generator.HasNext()) {
    auto separators = sep_generator.Next(100000 * G.N / full_graph.N + 1);
    ...
    for (const Separator &separator : separators) {
      ...
      if (upper <= search_lbnd || lower == upper) {
        if (lower == upper && G.N > 25) {
          auto hash_node = hash_cache.Insert(G.adj);
          if (hash_node) {
            hash_node->td = lower;
            hash_node->root = G.LocalIndex(root);
            //      std::cout << "Inserted the graph into the hash cache." <<
            //      std::endl;
          } else
            std::cout << "Failed to insert graph of size " << G.N

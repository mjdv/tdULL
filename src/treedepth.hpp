...
class Treedepth {
 public:
  const Graph &G;
  int search_lbnd, search_ubnd;
  Treedepth(const Graph &G, int search_lbnd, int search_ubnd)
      : G(G), search_lbnd(search_lbnd), search_ubnd(search_ubnd) {}
 ...
 protected:
  ...
  inline void SeparatorIteration(const Separator &separator, int &new_lower) {
    CheckTime();
    ...
    for (auto &&H : cc) {
      auto tuple = Treedepth(H, search_lbnd_sep, search_ubnd_sep).Calculate();
  ...
  std::tuple<int, int, int> VertexLoop() {
    std::vector<int> vertices;
    vertices.reserve(G.N);

    // Only add non-leaf vertices.
    for (int v = 0; v < G.N; ++v)
      if (G.Adj(v).size() > 1) vertices.emplace_back(v);
    return VertexLoop(std::move(vertices));
  }

  std::tuple<int, int, int> VertexLoop(std::vector<int> &&vertices) {
    assert(vertices.size());
    int new_lower = G.N;
    if (G.N == full_graph.N)
      std::cerr << "full_graph: bounds before vertex loop " << lower
                << " <= td <= " << upper << "." << std::endl;

    // Filter out dominated vertices.
    {
      size_t c = vertices.size();
      vertices = G.NonDominatedVertices(vertices);
      if (G.N == full_graph.N)
        std::cout << "full_graph: " << vertices.size() << " / " << c
                  << " are non dominated vertices." << std::endl;
    }
    ...
    for (size_t i = 0; i < vertices.size(); i++) {
      VertexIteration(vertices[i], new_lower);
  ...
  inline void VertexIteration(int vertex, int &new_lower) {
    CheckTime();
    ...
    for (auto &&H : cc) {
      auto tuple = Treedepth(H, search_lbnd_v, search_ubnd_v).Calculate();
...
void reconstruct(const Graph &G, int root, std::vector<int> &tree, int td) {
  ...
  int new_root = std::get<2>(Treedepth(G, td, G.N).Calculate());
  assert(new_root > -1);
...
std::pair<int, std::vector<int>> treedepth(const Graph &G) {
  ...
  int td = std::get<1>(Treedepth(G, 1, G.N).Calculate());

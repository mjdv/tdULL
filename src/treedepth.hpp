...
class Treedepth {
 public:
  ...
  std::tuple<int, int, int> Calculate(int search_lbnd, int search_ubnd,
    ...
    // If G has a leaf, we can do directed separator generation from that
    // leaf: that gives the same separators as the ordinary separator
    // generation. (Though possibly in a different order.) If the graph does
    // not have a leaf, then we do ordinary separator generation.

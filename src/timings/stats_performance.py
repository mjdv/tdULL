...
print('Example,Treedepth,{}'.format(','.join(
    [fn.split('/')[0] for fn in fns_glob])),
      file=sys.stderr)
for example in sorted(examplestats.keys()):
    ...
    print('{},{:<3},{}'.format(
        example, td_min, ','.join([str(time).rjust(6) for time in times])),
          file=sys.stderr)
...
for fn in fns_glob:
    ...
    if total_solved:
        print('{:<25}\t\t{}\t\t{}'.format(
            fn.split('/')[0], total_solved, total_time / total_solved))
